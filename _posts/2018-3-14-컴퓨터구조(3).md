...
가장 빈번하게 쓰는곳을 개선해야 한다.(Amdahl's Law) *Amdahl's Law : 컴퓨터 시스템의 일부를 계산할때 전체적으로 얼마만큼의 최대 성능 향상이 있는지 계산 하는데 사용한다.[1]
여러개의 명령어를 병렬적으로 처리한다.
자동차 생산라인처럼 단계적으로 가공하는 방법처럼 명령어도 단계적으로 처리함으로써 빠르게 처리할 수 있다.(Pipelining)
성능 개선의 걸림돌을 예측할 수있다. 예를들어, 조건문은 실행되기전까지 아무것도 결정지을 수 없다. 조건을 결정시 이 부분에 성능 결점이 생기는데 이러한 부분을 예측할 수 있다.
